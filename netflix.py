...
def loadset():    
    ...
    for count,row in enumerate(reader):
       ...
       if count >0:
           netFlix=float(NetflixRating)
           imdbceil=ceil(float(imdbRating))    
           Type_set[Type]=netFlix+Type_set[Rated]
           Type_count[Type]=Rated_count[Type]+1
           Rated_set[Rated]=netFlix+Rated_set[Rated]
           Rated_count[Rated]=Rated_count[Rated]+1      
           list_into_set(genre_set, Genre,netFlix,genre_count)
           Director_set[Director]=netFlix+Director_set[Director]
           Director_count[Director]=1+Director_count[Director]
           list_into_set(Writer_set,Writer,netFlix,Writer_count)
           list_into_set(Actors_set,Actors,netFlix,Actors_count)
           list_into_set(Language_set, Language,netFlix,Language_count)
           list_into_set(Country_set,Country,netFlix,Country_count)
