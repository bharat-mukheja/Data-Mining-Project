...
def save_file(data):
    global file_counter
    file_name = 'All_Users_Stats'+str(file_counter)+'.txt'
    file_counter+=1
...
for user in Users:
    ...
    #modified_df = movies_df[list(map(lambda v: v in list(map(int,Users[user])), list(movies_df.Number)))]
        ...
        print("saved")
        mycsvfile = ld()
        test = eng.netflix(nargout=14)
        user_prediction_stats  = list(test[:8])+[y for sublist in [x._data.tolist() for x in test[8:]] for y in sublist]
    ...
    '''if temp_counter==1000:
        ...
        temp_counter = 0
'''
with open('100_Users_Stats_with_p_r.txt','w') as statsfile:
